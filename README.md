...
Docs Agent workflow call sites use the generic Homeboy full-run workflow and the committed `docs-agent/codebox-homeboy-runner` recipe in `ci/docs-agent-runner-recipe.json`. The recipe declares product-level runner inputs such as `runtime`, `profile`, `capabilities`, `component_contracts`, and `ability_requirements`; the resolver maps those fields into the current Homeboy workflow inputs. Publication remains runner-owned while agents edit the provided workspace.
