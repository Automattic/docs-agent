...
The runner migration is tracked in [Automattic/docs-agent#100](https://github.com/Automattic/docs-agent/issues/100). Docs Agent workflow call sites use the generic Homeboy full-run workflow, with the Codebox/Homeboy contract centralized behind the `docs-agent/codebox-homeboy-runner` recipe in `ci/docs-agent-runner-recipe.json`. The recipe declares product-level runner inputs such as `runtime`, `profile`, `capabilities`, `component_contracts`, and `ability_requirements`; the resolver maps those into the current Homeboy workflow inputs. Publication remains runner-owned while agents only edit the provided workspace.
