...
Docs Agent workflow call sites use the generic Homeboy full-run workflow: `Extra-Chill/homeboy-extensions/.github/workflows/runtime-agent-full-run.yml@main`. Docs Agent keeps its Codebox bundle runtime profile behind the committed `docs-agent/codebox-agent-bundle` recipe in `ci/docs-agent-runner-recipe.json`. Workflows run `ci/resolve-docs-agent-runner-recipe.php` and pass the resolved compatibility fields to Homeboy Extensions: `runtime_provider`, `runtime_ref`, `runtime_profile`, `runtime_profiles`, `runtime_dependencies`, `runtime_components`, `ability_requirements`, and `runtime_config`. This migration is tracked in Automattic/docs-agent#100 and depends on the generic full-run support available on `Extra-Chill/homeboy-extensions@main`.

Data Machine remains present only inside the selected recipe for the current Docs Agent bundles, which still execute through `datamachine/run-agent-bundle`. Bundle selection is expressed through `runtime_execution` rather than legacy `bundle_path` inputs. Writable workspace scope is declared through the public `writable_paths`, `allowed_repos`, and `runner_workspace` inputs; Docs Agent does not mount sandbox policy plugins or pass sandbox policy defines.
