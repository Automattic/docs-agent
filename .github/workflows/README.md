...
Docs Agent workflow call sites use the generic Homeboy full-run workflow: `Extra-Chill/homeboy-extensions/.github/workflows/runtime-agent-full-run.yml@main`. Docs Agent stores its Codebox/Homeboy runner contract in the committed `docs-agent/codebox-homeboy-runner` recipe at `ci/docs-agent-runner-recipe.json`. The recipe declares product-level runner fields for Docs Agent, including `runtime`, `profile`, `capabilities`, `component_contracts`, and `ability_requirements`; `ci/resolve-docs-agent-runner-recipe.php` maps those fields to the current Homeboy Extensions workflow inputs. This support is tracked in Automattic/docs-agent#100 and uses the generic full-run support available on `Extra-Chill/homeboy-extensions@main`.
