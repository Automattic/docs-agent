...
Docs Agent workflow call sites use the generic Homeboy full-run workflow: `Extra-Chill/homeboy-extensions/.github/workflows/runtime-agent-full-run.yml@main`. Docs Agent keeps its Codebox/Homeboy runner contract behind the committed `docs-agent/codebox-homeboy-runner` recipe in `ci/docs-agent-runner-recipe.json`. The recipe declares product-level runner fields for Docs Agent, including `runtime`, `profile`, `capabilities`, `component_contracts`, and `ability_requirements`; `ci/resolve-docs-agent-runner-recipe.php` maps those fields to the current Homeboy Extensions workflow inputs. This migration is tracked in Automattic/docs-agent#100 and depends on the generic full-run support available on `Extra-Chill/homeboy-extensions@main`.
