...
$assert( in_array( 'docs-agent.workspace-editing', $recipe['capabilities'] ?? array(), true ), 'Runner recipe must declare Docs Agent runner capabilities.' );
$assert( isset( $recipe['component_contracts'] ) && is_array( $recipe['component_contracts'] ), 'Runner recipe must declare component contracts instead of component paths.' );
$assert( in_array( 'docs-agent.bundle-execution', $recipe['ability_requirements'] ?? array(), true ), 'Runner recipe must declare Docs Agent ability requirements without runtime ability names.' );
...
foreach ( array( 'Docs Agent Runner Recipe', 'Extra-Chill/homeboy-extensions@main', 'Automattic/docs-agent#100', 'runtime-agent-full-run.yml', 'docs-agent/codebox-homeboy-runner', 'resolve-docs-agent-runner-recipe.php', 'capabilities', 'component_contracts', 'ability_requirements' ) as $migration_note_text ) {
